...
export function MausamApp() {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col pb-24">
      ...
      {tab === "forecast" && <ForecastScreen profile={profile} />}
      {tab === "alerts" && <AlertsScreen profile={profile} />}
      {tab === "saved" && <SavedScreen activeId={locationId} onSelect={setLocationId} />}
      {tab === "profile" && (
        <ProfileScreen profile={profile} onSelectProfile={setActiveId} />
      )}
